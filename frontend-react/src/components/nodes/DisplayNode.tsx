    ...
    <BaseNode {...props}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexShrink: 0 }}>
          <Text type="secondary" style={{ fontSize: 13, fontWeight: 600, color: '#475569' }}>VISUAL_OUTPUT</Text>
          {displayContent && (
             <Tooltip title="固化为资产">
        ...
        {/* 🌟 核心防御：自适应宽高，绝对定位包裹 */}
        <div className="nodrag" style={{ flex: 1, position: 'relative', background: mediaType === 'text' ? '#f8fafc' : '#0f172a', borderRadius: 8, border: '1px solid #cbd5e1', overflow: 'hidden', minHeight: 120 }}>
          {displayContent ? (
            mediaType === 'video' ? (
               <video src={displayContent} controls autoPlay loop muted style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'contain' }} />
            ) : mediaType === 'image' ? (
               <img src={displayContent} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'contain' }} alt="Display" />
            ) : (
               <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', padding: 12, overflowY: 'auto', fontSize: 13, color: '#1e293b', whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
                 {displayContent}
               </div>
            )
          ) : (
